...
def menuNav(user, id):
    ...
    while not (choiceMenu in options):
        ...
        if (choiceMenu in options):
            ...
            # log chosen option
            log.PrepareLog(indexId, f"{self.username}", "Menu navigation option chosen", "/", "no")
            id = indexId
            if (choiceMenu == "0"):
                exit()
            for x in options:
        ...
        # else statement to log incorrect user input and print menu again
            ...
            log.PrepareLog(indexId, f"{self.username}", "Menu navigation incorrect input", "input: '%s' used as main menu choice" % user_input, "no")
            id = indexId
            ClearConsole()
...
def OptionNavigation(options, user, id):
    if (options == "1"):
        indexId = log.SystemCounter(id)
        # log user choice
        log.PrepareLog(indexId, f"{self.username}", "Option navigation option chosen", "input: '%s' used as choice" % options, "no")
        id = indexId
    ...
    if (options == "2"):
        if (user.role == "superadmin"):
            indexId = log.SystemCounter(id)
            # log user choice
            log.PrepareLog(indexId, f"{self.username}", 'Option navigation option chosen', 'input: "%s" used as choice by Super Admin' % options, 'no')
            id = indexId
